...
int main(int argc, char** argv) {
    ...
    int blocks = FIXME; // Set the number of blocks to an appropriate value now that each GPU is only working on `N / device_count` values.

    // Allow for asynchronous execution by launching all kernels first
    // and then synchronizing on all devices after.
    for (int i = 0; i < device_count; ++i) {
        cudaSetDevice(i);
        calculate_pi<<<blocks, threads_per_block>>>(d_hits[i], FIXME); // The kernel now expects a `device` integer, so each GPU can generate different random values.
    }
    ...
    for (int i = 0; i < device_count; ++i) {
        cudaSetDevice(i);
        cudaMemcpy(FIXME, FIXME, sizeof(int), cudaMemcpyDeviceToHost); // Fix so that each GPU copies its `hits` counter back to the host.
    }
