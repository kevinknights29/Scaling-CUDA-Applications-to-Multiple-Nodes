...
__global__ void histogram_kernel(const int* input, int* histogram, int N)
{
    ...
    if (idx < N) {
        ...
        int local_histogram_index = global_histogram_index - FIXME;
...
int main(int argc, char** argv) {
    ...
    int* d_concatenated_histogram = (int*) nvshmem_malloc(FIXME);
    ...
    nvshmem_int_collect(NVSHMEM_TEAM_WORLD, d_concatenated_histogram, d_histogram, FIXME);
